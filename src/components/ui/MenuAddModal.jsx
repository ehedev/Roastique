...
const MenuAddModal = ({ show, onHide, onSave }) => {
    const { register, handleSubmit, formState: { errors } } = useMenuForm(show, null);
    ...
    return (
        <Modal show={show} onHide={onHide} centered backdrop="static" size='lg'>
            <Modal.Header className="border-0 pt-4 px-4 pb-0 d-flex justify-content-between align-items-center">
                <Modal.Title className="fw-bold" style={{ color: 'var(--text-primary)' }}>Add New Item</Modal.Title>
                <Button variant="link" onClick={onHide} className="closeButton text-dark p-0"><FiX size={24} /></Button>
            </Modal.Header>
            <Modal.Body className="p-4">
                <Form onSubmit={handleSubmit(submitHandler)} id="menuForm">
                    <Row className="g-3">
                        <Col md={6}>
                            <Form.Group>
                                ...
                                <Form.Control 
                                    type="text" 
                                    {...register('name')} 
                                    className={`rounded-0 p-2 ${errors.name ? 'is-invalid' : ''}`}
                                />
                        ...
                        <Col md={6}>
                            <Form.Group>
                                ...
                                <Form.Select 
                                    {...register('category')}
                                    className={`rounded-0 p-2 ${errors.category ? 'is-invalid' : ''}`}
                                >
                        ...
                        <Col md={6}>
                            <Form.Group>
                                ...
                                <Form.Control 
                                    ...
                                    step="0.01"
                                    {...register('price')} 
                                    className={`rounded-0 p-2 ${errors.price ? 'is-invalid' : ''}`}
                                />
                        ...
                        <Col md={6} className="d-flex align-items-center mt-4">
                            <Form.Check 
                                ...
                                label={<span className="fw-bold" style={{ color: 'var(--text-primary)' }}>Item is currently Available</span>}
            ...
            <Modal.Footer className="border-0 pb-4 px-4 pt-0">
                ...
                <Button 
                    type="submit" 
                    form="menuForm"
                    className="rounded-0 px-4 border-0 fw-bold shadow-sm"
